package com.bulade.donor.framework.operatelog.core.aop;
...
public class OperateLogAspect {
    ...
    private void log(ProceedingJoinPoint joinPoint, OperateLog operateLog, Operation operation,
                     LocalDateTime startTime, Object result, Throwable exception) {
        try {
            if (!isLogEnable(joinPoint, operateLog)) {  // 判断不记录的情况
                return;
            }
            // 真正记录操作日志
            var operateLogObj = new com.bulade.donor.framework.operatelog.core.service.OperateLog();
            ...
            log.error("[log][记录操作日志时，发生异常，其中参数是:]");
            log.error("[log][joinPoint({})]", joinPoint);
            log.error("[log][operateLog({})]", operateLog);
            log.error("[log][apiOperation({})]", operation);
            log.error("[log][result({})]", result);
            log.error("[log][exception({})]", ex.getMessage());
        }
    }

    private void fillUserFields(com.bulade.donor.framework.operatelog.core.service.OperateLog operateLogObj) {
    ...
    private static void fillModuleFields(
        com.bulade.donor.framework.operatelog.core.service.OperateLog
            operateLogObj,
    ...
    private static void fillModuleType(
        com.bulade.donor.framework.operatelog.core.service.OperateLog operateLogObj,
    ...
    private static void fillMethodFields(
        ...
        if (result instanceof CommonResponse<?> commonResult) {
            operateLogObj.setResultCode(commonResult.getCode());
            operateLogObj.setResultMsg(commonResult.getMessage());
        } else {
            operateLogObj.setResultCode(ResultCodeEnum.SUCCESS.getCode());
        }
        // （异常）处理 resultCode 和 resultMsg 字段
        if (exception != null) {
            operateLogObj.setResultCode(ResultCodeEnum.ERROR.getCode());
    ...
    private static String obtainResultData(Object result) {
        ...
        if (result instanceof CommonResponse) {
            result = ((CommonResponse<?>) result).getData();
        }
